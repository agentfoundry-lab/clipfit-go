	...
	return []map[string]any{
		{
			...
			"description": "FIRST STEP for every existing-file edit. Validate structured operations against the current file and return hunks for the exact platform-normalized output bytes plus a short-lived preview_id. Does not write. Distant edits in input already using the platform EOL remain separate hunks; line-ending normalization is included in the preview. Non-Windows builds emit LF and Windows builds emit CRLF. Fails closed if any find is missing or ambiguous; split into smaller previews only when the server returns an explicit response safety-limit error. Prefer replace_block with a unique verbatim anchor above find.",
